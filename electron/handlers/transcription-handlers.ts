...
export function registerTranscriptionHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (event, filePath: string, projectId: string): Promise<void> => {
      ...
      try {
        ...
        db.prepare('UPDATE projects SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(
          'processing',
          projectId,
        )
   ...
   */
  ipcMain.handle('transcription:getByProjectId', async (_event, projectId: string): Promise<Transcription> => {
    const db = databaseService.getDatabase()
  ...
  ipcMain.handle(
    ...
    async (_event, segmentId: string, text: string): Promise<Segment> => {
      ...
      // 更新されたセグメントを取得
      const segmentRow = db.prepare('SELECT * FROM segments WHERE id = ?').get(segmentId) as SegmentRow | undefined
      ...
      const allSegments = db
        .prepare('SELECT text FROM segments WHERE transcription_id = ? ORDER BY sequence_number ASC')
        .all(segmentRow.transcription_id) as { text: string }[]

      const fullText = allSegments.map((s) => s.text).join(' ')

      db.prepare('UPDATE transcriptions SET content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(
        fullText,
        segmentRow.transcription_id,
      )
