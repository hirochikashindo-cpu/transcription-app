...
### 2.4 文字起こし・AI処理
- **文字起こし (Phase 1)**: whisper.cpp (ローカル実行)
  - モデル: Whisper Large v3 Turbo (~809MB)
  - APIコスト不要、プライバシー保護、オフライン動作
  - GPU加速対応（Metal/CUDA）
- **AI要約 (Phase 3)**: ローカルLLM（ollama等）
  - コスト削減、プライバシー保護
  - 推敲・精度向上機能を含む
...
#### 3.3.0 文字起こし結果の推敲・精度向上
**必須要件**:
- ローカルLLM（ollama等）を使用した文字起こし結果の推敲
- 専門用語や固有名詞の修正
- 句読点・言い回しの最適化
- コスト不要・プライバシー保護
...
**プロンプト例**:
```
以下は音声認識の文字起こし結果です。誤字脱字や不自然な表現を修正してください。
専門用語: [カスタム辞書から取得]
...
### 4.3 プライバシー
- **完全ローカル処理**: 音声・文字起こしデータは外部送信しない
- **whisper.cpp**: ローカル実行のため音声データが外部に送信されない
- **ローカルLLM**: 推敲・要約もローカル処理、プライバシー完全保護
